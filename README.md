...
- [x] **Intro to Pointers:** Implement a function that reverses an array in-place using only pointer arithmetic (no indexing).
- [x] **Pointer Arithmetic & Array Decay:** Write `matrixFlatten(int** mat, int r, int c)` returning a heap-allocated 1D array using raw pointer traversal.
- [x] **References vs Pointers:** Write a generic swap without `<algorithm>`, once with pointers and once with references; benchmark with `std::chrono`.
- [ ] **Null, Void & Multiple Indirection:** Implement a `void*`-based memory pool that stores heterogeneous types and retrieves them via casting.

### Module 2: Dynamic Memory & Smart Pointers
- [x] **new / delete, heap vs stack:** Build a dynamic stack (LIFO) using raw `new`/`delete`; detect and fix a deliberately seeded memory leak using Valgrind.
- [x] **unique_ptr:** Implement a singly linked list where each node owns the next via `unique_ptr`; write insert, remove, print.
- [ ] **shared_ptr & weak_ptr:** Model a graph with shared nodes; use `weak_ptr` to break a cycle — verify with a custom destructor print trace.
- [ ] **std::move & Move Semantics:** Write a Buffer struct holding heap data; implement move constructor/assignment manually; confirm zero copies with trace logging.

### Module 3: Functions — Advanced
- [x] **Function Pointers:** Build a dispatch table (`std::array` of function pointers) mapping `char` operators to arithmetic lambdas.
- [x] **std::function & Callable Wrappers:** Implement a `Pipeline<T>` that chains `std::function<T(T)>` transforms and applies them sequentially to a value.
- [x] **Lambdas & Captures:** Write a memoization wrapper using a lambda capturing a `std::unordered_map`; test with Fibonacci.
- [x] **constexpr Functions:** Implement compile-time integer `sqrt` and verify result is a `constexpr` using a `static_assert`.

### Module 4: Templates
- [x] **Function Templates:** Write a template `clamp<T>(T val, T lo, T hi)` and verify it works for `int`, `double`, and a custom comparable type.
- [x] **Class Templates (non-OOP usage):** Implement a template `CircularBuffer<T, N>` (stack-allocated, fixed-size) with push, pop, full, empty.
- [x] **Template Specialization:** Write a `TypeInfo<T>` struct with a `name()` specialization for `int`, `double`, `char`, and a generic fallback.
- [x] **Variadic Templates:** Implement `print(args...)` that prints all arguments separated by " | " using fold expressions (C++17).
- [x] **SFINAE & if constexpr:** Write a template `serialize<T>` that outputs `to_string(val)` for arithmetic types and `"[non-scalar]"` otherwise, selected at compile time.
...
- [x] **Placement new:** Implement a fixed-size arena allocator using a `char[]` buffer and placement new; support `allocate<T>()` and manual destructor calls.
...
- [x] **RAII Pattern (non-OOP):** Wrap a raw `FILE*` in a non-inheriting struct with a destructor; ensure it closes on all exit paths including exceptions.
...
- [ ] **std::optional, std::variant, std::any:** Implement a config parser returning `std::variant<int, float, string, std::monostate>`; use `std::visit` with a lambda overload set.
...
- [ ] **Concurrency (std::thread, std::mutex, std::async):** Parallelize a `std::accumulate` over chunks using `std::async`; guard a shared result with `std::mutex`; compare runtime vs serial.
...
- [ ] **Exceptions — advanced:** Write a function chain where exceptions propagate through 3 levels; catch by base type; re-throw with `std::throw_with_nested`; unwind with `std::rethrow_if_nested`.
